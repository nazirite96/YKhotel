...
public class BbsDAO {
	...
	/*이전글 다음글 찾기 메서드 rownum이용*/
	/*일단먼저 rownum찾기*/
	public int findrownum(int idx) {
		try {
			conn=semidb.DB.getConn();
	
			String sql=	"select * from ( select rownum as rnum,a.* from ( select * from yk_bbs order by bbs_idx)a)b where bbs_idx=?";
	...
	/*이전글 다음글 찾기  없으면 0*/
public BbsDTO bbsPrevAndNext(int rnum) {
		
		try {
		
			conn=semidb.DB.getConn();
					
			String sql="select * from ( select rownum as rnum,a.* from ( select * from yk_bbs order by bbs_idx)a)b where rnum=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1,rnum);
			rs=ps.executeQuery();
	...
	/*검색된페이지총갯수*/
	public int getTotalCntfromSearch(String sel,String tf) {
	
try {
			
			conn=semidb.DB.getConn();
			
			String sql="select count(*) from ( select * from yk_bbs where "+sel+" like '%"+tf+"%' ) a";
			ps=conn.prepareStatement(sql);
