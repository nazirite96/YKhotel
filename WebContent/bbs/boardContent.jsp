...
document.getElementById(cidx).innerHTML="<fieldset class='recomfield'><div><%=sid%></div>"+
"<input type='text'  name='com_recontent' class='recominput'><input  type='hidden'  name='hiddenref' value='"+cref+"'>"
+"<input type='submit' value='등록' class='recombt'>&nbsp;<input type='button' value='취소' onclick='resetcom("+cidx+");'  class='recombt2'></fieldset> ";
...
<section>
<article>
<img src="/semi/img/3bbb.jpg"  class="contentimg">
</article>
</section>

<div class="comtitle">Our Travel Stories</div>
...
<tfoot align="center">
<tr>
<td class="likeimg" colspan="6">
<a href="boardLikenum_ok.jsp?idx=<%=bdto.getBbs_idx()%>"  class="likeAtag">
<img src="/semi/img/bh.jpg" style="width:45px;" >
</a>
</td>
</tr>
<tr>
<td class="likenum" colspan="6" >
...
if (bdto.getBbs_imgname().equals("nono")){   //파일이름이 nono이면 사진파일을 올리지않았다는뜻. 사진띄우지않음
	...
	<img src="/semi/upload/<%=bdto.getBbs_imgname() %>"  class="conimg">
...
<section>
	<article>
	 	<form name="commentForm"   action="/semi/bbs/com/comWrite_ok.jsp"  onsubmit="return comNullno(this);">
	 	<input type="hidden" name="hiddenbbsidx" value="<%=bdto.getBbs_idx()%>">
	 	
	 	<table class="tabcom"  >
	 	<thead>
	 	<tr>
	 	<th colspan="3" align="left" class="thcom">댓글(<%=comCnt %>)</th>
	 	</tr>	
	 	</thead>
...
%>
	 	...
	 	if(sid.equals(carr.get(i).getCom_id())){
	 		%>
	 		<a href="/semi/bbs/com/comDelete_ok.jsp?idx=<%=cidx%>&lev=<%=carr.get(i).getCom_lev()%>&ref=<%=carr.get(i).getCom_ref()%>" >삭제</a>
	 		<% 
	 	}
	 	%>
...
<input type="hidden" name="imgname" value="<%=bdto.getBbs_imgname() %>">
...
if(sessionId==null  || !sessionId.equals(bdto.getBbs_id())){
	%>
	<div>
<input type="button" value="목록보기" onclick="location.href='/semi/bbs/boardList.jsp'" >
<input type="button" value="답글달기" onclick="goRewrite();">
 </div>
	<% 
