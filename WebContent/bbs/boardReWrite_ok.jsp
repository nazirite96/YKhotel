...
<%@page import="bbs.*" %>
 ...
 <%
 /*게시글 답글 쓰기 처리페이지*/
  System.out.println("rewrite이미지업로드처리입장");
//공용 upload폴더에 이미지 넣어야함.
...
String ref_s=multi.getParameter("bbs_ref");
String lev_s=multi.getParameter("bbs_lev");
String sunbun_s=multi.getParameter("bbs_sunbun");

Integer ref=Integer.parseInt(ref_s);
Integer lev=Integer.parseInt(lev_s);
Integer sunbun=Integer.parseInt(sunbun_s);
...
int count=bdao.bbsReWrite(dto);

if(count>0){
	%>
	<script>
	window.alert('답글이 등록되었습니다.');
