...
<script>
	function bbcUsed() {
		...
		if (bbc[0].checked == true) {
			document.getElementById('bbcuse').innerHTML = (<%=roomDto.getRoom_price() * calDateDays%>+ (bbc[0].value * 10000))+'원';
			document.all.reser_totalprice_s.value = ''+(<%=roomDto.getRoom_price() * calDateDays%>+ (bbc[0].value * 10000));
			document.all.reser_bbc.value = '1';
		} else {
			document.getElementById('bbcuse').innerHTML =(<%=roomDto.getRoom_price() * calDateDays%>)+'원';
			document.all.reser_totalprice_s.value = ''+ (<%=roomDto.getRoom_price() * calDateDays%>);
...
<body>
	...
	<section>
		<article class="reser_article">
			...
			<table class="resertab" border="1" >
				<tr>
					<td rowspan="6" class="imgtd"><img
						src="<%=roomDto.getRoom_img()%>"></td>
				</tr>
				<tr>
					<td class="tdsmall tdcheckin" >체크인</td>
					<td class="checkin tdsmall2" colspan="3"><%=strdate%> 14:00시</td>
					<td class="tdsmall tdcheckin" >체크아웃</td>
					<td class="checkin tdsmall2"   colspan="2"><%=enddate%> 11:00시</td>
